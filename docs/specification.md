---
title: シャドーイングアプリの仕様
description: シャドーイング練習アプリの機能仕様と音声設定仕様
---
...
- シャドーイングでは、各ターンで次のことが起こる
    - アプリが、課題作成時に保存された音声モデルと音声の種類を使用して各ターンの音声を読み上げる
...
- シャドーイングが終了すると、採点が行われる
    - アプリは、STTプロバイダを使用してユーザーの読み上げた音声を書き起こす
        - 現在の実装では whisper-1 を使用する
        - 将来、同じインターフェースを持つ別の音声認識モデルへ差し替え可能
...
- ユーザーは以下の項目を設定できる。これらは、課題作成時に音声を作成するときの設定を意味する。
    ...
    - 音声モデル：gpt-4o-mini-tts、MAI-Voice-1、MAI-Voice-2 を選択できる
    - 音声の種類：選択した音声モデルに対応する音声のみを選択できる
        - gpt-4o-mini-tts: alloy, ash, ballad, coral, echo, fable, onyx, nova, sage, shimmer, verse
        - MAI-Voice-1: Jasper, June, Grant, Iris, Reed, Joy
        - MAI-Voice-2: Ethan, Grant, Harper, Iris, Jasper, Olivia
...
- MAI-Voice-1またはMAI-Voice-2を利用する場合はMicrosoft FoundryのAPIキーとエンドポイントURLが必要
- マイクアクセス許可が必要
