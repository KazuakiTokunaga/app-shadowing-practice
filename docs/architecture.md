---
title: シャドーイングアプリ アーキテクチャ設計
description: シャドーイング練習アプリの構成、データ設計、API設計、音声処理設計
---
...
個人使用向けのローカル実行型シャドーイングアプリ。英語学習者がTTS/STTモデルを活用した音声処理でシャドーイング練習を行うことができる。
...
```text
┌─────────────────┐    HTTP     ┌─────────────────┐    HTTPS    ┌────────────────────┐
│   Web Browser   │ ◄────────► │  Python Server  │ ◄────────► │ External Model APIs │
│  (Chrome Only)  │             │   (Backend)     │             │ OpenAI / Foundry    │
└─────────────────┘             └─────────────────┘             └────────────────────┘
         │                               │
...
アプリケーション側の処理と外部モデルAPIの呼び出しは、次の境界で分離する。

- `SpeechService`: 課題音声の生成、ファイル保存、TTSプロバイダ選択
- `TranscriptionService`: 録音音声の書き起こしとSTTプロバイダ選択
- `TurnService`: 課題本文のターン分割
- `ScoringService`: 書き起こし結果と課題本文の一致率計算
- `services/model_providers/`: OpenAI、MAI、Whisperなど外部モデルAPIの呼び出し実装

TTS/STTプロバイダは共通インターフェースを持つため、アプリケーション側のロジックを変えずに別モデルの実装を追加できる。
...
FOUNDRY_ENDPOINT_URL=your_foundry_endpoint_url_here
...
CREATE TABLE exercises (
    ...
    speech_model VARCHAR(50) DEFAULT 'gpt-4o-mini-tts',  -- 音声モデル（課題作成時の設定値）
...
POST   /api/exercises          # 課題作成（ターン分割と音声生成）
GET    /api/exercises/{id}     # 課題詳細取得（speech_rate, speech_model, speech_voice含む）
...
### データ保護
- **ローカル実行**: TTS/STTプロバイダが呼び出す外部モデルAPI以外への外部通信なし、データは全てローカル保存
- **音声データ**: 
  - 生成された音声ファイルはローカルに保存（課題削除時に完全削除）
  - ユーザー録音データはSTTプロバイダへの送信後は即座に削除、永続化しない
...
2. **データベース保存**: 
  ...
  - exercises テーブルの speech_model カラムに音声モデルを保存
   - exercises テーブルの speech_voice カラムに音声の種類を保存
3. **音声生成**: SpeechServiceが音声モデルに対応するTTSプロバイダを選択し、設定された速度と音声タイプで音声を生成
